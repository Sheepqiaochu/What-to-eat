...
public class ClientRatefood extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonRateSubmit.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View view) {
                ...
                synchronized (ClientRatefood.this) {
                    try {
                        ClientRatefood.this.wait();
                ...
                if(done) {
                    ...
                    Intent intent = new Intent(ClientRatefood.this, Login.class);
                    startActivity(intent);
                }
                else
                    Toast.makeText(ClientRatefood.this, "评分提交失败！", Toast.LENGTH_SHORT).show();
        ...
        new Thread(new Runnable(){
            public void run(){
                ...
                try{
                    ...
                    for(int i=0;i<10;i++){
                        point[i]=rateFood[i].getExcellence();
                        if(point[i]==-1) {
                            ClientRatefood.this.notifyAll();
                            throw new Exception();
                        }
                    }
                    map.put("point",point);
                    ...
                    Response response=client.newCall(request).execute();
                    String responseData=response.body().string();
                    parseJSONWithGSON_2(responseData);        //用gson处理json数据
                }catch (Exception e){
                    e.printStackTrace();
                }
            }}
    ...
    private void parseJSONWithGSON_2(String jsonData){    //用gson处理json数据
